...
score_lang = [(p, q) for p, q in zip(df.mean(), col_names)]
score_lang = sorted(score_lang, key=lambda pair: pair[0], reverse=True)
keep_lang = [x[1] for x in score_lang if (x[0] > 0.5 and x[1] != "en")]

lang_seqs = np.array_split(keep_lang, 30)
np.save("lang_seqs.npy", lang_seqs)
# langs = np.load("lang_seqs.npy", allow_pickle=True)[seq_id]


np.save("remove_lang.npy", remove_lang)
print(f"Remove Languages: {remove_lang}")
# last_10_langs = [x[1] for x in score_lang[:10]] + ['en']
# df = pd.read_csv("play_bt_all_sim_bleu.csv")
#
# for col_name in col_names:
#     if col_name not in last_10_langs:
#         del df[col_name]
#
# df.to_csv("last_10_sim.csv")
