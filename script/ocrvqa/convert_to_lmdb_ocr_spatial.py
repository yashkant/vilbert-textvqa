...
OCR_LMDB_OCRVQA = "/srv/share/ykant3/ocr-vqa/features/lmdbs/ocrvqa_ocr_spatial.lmdb"
...
if __name__ == "__main__":
    ...
    obj_feature_files = glob.glob(OBJ_FEATURES_OCRVQA + "/**", recursive=True)
    obj_feature_files = [path for path in obj_feature_files if path.endswith(".npy")]
    ...
    assert len(ocr_feature_files) == len(obj_feature_files)
    ...
    with env.begin(write=True) as txn:
        for infile in tqdm.tqdm(ocr_feature_files):
            try:
            ...
            obj_file = os.path.join(OBJ_FEATURES_OCRVQA, os.path.split(infile)[-1])
            obj_reader = np.load(obj_file, allow_pickle=True)
