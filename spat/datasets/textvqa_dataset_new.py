...
class TextVQADataset(Dataset):
    ...
    def __init__(
        ...
        self.obj_features_reader = ImageFeaturesH5Reader(
            features_path=self.textvqa_obj.format(format_str),
            in_memory=True
        )
        self.ocr_features_reader = ImageFeaturesH5Reader(
            features_path=self.textvqa_ocr.format(format_str),
            in_memory=True
        )
    ...
    def process_spatials(self):
        ...
        with mp.Pool(self.processing_threads) as pool:
            results = list(
                tqdm(pool.imap(SpatialProcessor, pad_obj_ocr_bboxes_list), total=len(pad_obj_ocr_bboxes_list)))
