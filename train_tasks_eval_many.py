...
def get_parser():
    ...
    parser.add_argument(
        "--resume_dir", default="", type=str, help="Resume from checkpoint"
    )
    parser.add_argument(
        "--dynamic_attention",
        action="store_true",
    ...
    parser.add_argument("--eval_top", type=int, required=True)
...
def main():
    ...
    # dump evaluated ckpts
    dump_path = f"{args.resume_dir}/evaluated/"
    os.makedirs(dump_path, exist_ok=True)

    for i in tqdm(range(args.eval_top), total=args.eval_top):
        ckpt = f"{args.resume_dir}/vqa_rank_{topmost - i}.tar"
        if os.path.exists(ckpt):
            ckpts.append(ckpt)
            shutil.copy2(ckpt, dump_path)

    print(f"Evaluating on these: {ckpts}")
    ...
    for resume_file in ckpts:
        if os.path.exists(resume_file):
            model = M4C(mmt_config, text_bert_config)
            json_path = dump_path + os.path.split(resume_file)[1].split(".")[0] +".json"
...
def evaluate(
    ...
    # evalai_path = f"{save_dir}/evalai_{}"
    ...
    json.dump(evalai_results, open(json_path, "w"))
    # json.dump(final_results, open(preds_path, "w"))

    print(f"Dumped: {json_path}")
