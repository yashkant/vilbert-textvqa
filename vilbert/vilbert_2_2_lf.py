...
class VILBertForVLTasks(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels, dropout_prob=0.1, default_gpu=True):
        ...
        self.lf_ocr_features = nn.Linear(config.v_hidden_size + 604 + 300, config.v_hidden_size)
    ...
    def build_textvqa_scores(self, batch_dict, sequence_output_t, sequence_output_v):
        ...
        # late-fusion of FastText, PHOC and OCR output embeddings
        ocr_output = torch.cat([ocr_output, batch_dict["ocr_fasttext"], batch_dict["ocr_phoc"]], dim=2)
        ocr_output = self.lf_ocr_features(ocr_output)
