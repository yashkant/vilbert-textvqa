...
class TextCapsDataset(Dataset):
    def __init__(
        ...
        self.in_memory = extra_args.get("in_memory", True)
        logger.info(f"In memory feature loading: {self.in_memory}")

        if self.split == "test":
            self.obj_features_reader = ImageFeaturesH5Reader(
                features_path="/srv/share/ykant3/vilbert-mt/features/obj/test_obj.lmdb", in_memory=self.in_memory
            )
            self.ocr_features_reader = ImageFeaturesH5Reader(
                features_path="/srv/share/ykant3/vilbert-mt/features/ocr/test_ocr.lmdb", in_memory=self.in_memory
            )
        else:
            self.obj_features_reader = ImageFeaturesH5Reader(
                features_path=extra_args["features_h5path1"], in_memory=self.in_memory
            )
            self.ocr_features_reader = ImageFeaturesH5Reader(
                features_path=extra_args["features_h5path2"], in_memory=self.in_memory
            )

        self.spatial_features_reader = CacheH5Reader(
            features_path=f"/srv/share/ykant3/vilbert-mt/textcaps/cache/TextVQA_{split}_2_vocab_typetextcaps_dynamic_True_spatial.lmdb",
        ...
        if not os.path.exists(cache_path) or self.debug:
            ...
            # don't store cache unless we are using spatial-relations
            if not self.debug and self.needs_spatial:
            ...
            # self.cache_entries = cPickle.load(open(cache_path, "rb"))
            # self.cache_entries = self.process_cache(self.cache_entries)
            self.entries, _ = _load_dataset(dataroot, split, self.debug)
            if "processors" not in registry:
                self.processors = Processors(self._tokenizer, vocab_type=self.vocab_type)
                registry.processors = self.processors
    ...
    def process_cache(self, cache_entries):
        entries_dict = {}
        for entry in cache_entries:
            if entry["image_id"] not in entries_dict:
                entries_dict[entry["image_id"]] = entry["spatial_adj_matrix_shared"]
        return entries_dict
    ...
    def process(self):
        ...
        for entry in tqdm(self.entries, desc="Processing Entries"):
            # tensorize
            entry["caption_id"] = torch.tensor(entry["caption_id"])
            entry["image_height"] = torch.tensor(entry["image_height"])
            entry["image_width"] = torch.tensor(entry["image_width"])

            # process dummy question
            processed_question = self.processors.bert_processor({"question": ""})
    ...
    def __getitem__(self, index):
        ...
        if self.needs_spatial:
            entry["spatial_adj_matrix_shared"] = self.spatial_features_reader[image_id]
        ...
        entry["answers"] = [entry["caption_str"]]
...
class Processors:
    ...
    def __init__(self, bert_tokenizer, vocab_type="4k", only_registry=False):
        ...
        # decode-answers
        answer_config = edict()
        answer_config.max_copy_steps = 30
        answer_config.num_answers = 1
        answer_config.max_ocr_tokens = 50
        answer_config.vocab_type = vocab_type
        self.answer_processor = M4CCaptionProcessor(answer_config)
...
class ImageDatabase(torch.utils.data.Dataset):
    ...
    def _sort(self):
        sorted_data = sorted(self.data[self.start_idx:], key=lambda x: x["image_id"])
