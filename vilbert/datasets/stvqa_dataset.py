...
from vilbert.datasets.textvqa_dataset import TextVQADataset, ImageDatabase
...
def _load_dataset(path_holder, name):
    ...
    if name == "train" or name == "val":
        ...
        imdb_path = path_holder.format("test_task3")
    else:
        assert False, "data split is not recognized."

    logger.info(f"Loading IMDB for {name}")
...
class STVQADataset(TextVQADataset):
    ...
    def _set_attrs(self, task_cfg):
        keys = [
            ("stvqa_dataroot", None),
            ("max_seq_length", None),
            ("stvqa_ocr_features", None),
            ("stvqa_obj_features", None),
            ("stvqa_imdb", None),
            ("max_obj_num", None),
            ("max_ocr_num", None),
            ("mix_list", None),
    ...
    def __init__(
        ...
        self.obj_features_reader = ImageFeaturesH5Reader(
            features_path=self.stvqa_obj_features.format(split),
            in_memory=True
        )
        self.ocr_features_reader = ImageFeaturesH5Reader(
            features_path=self.stvqa_ocr_features.format(split),
            in_memory=True
        )
        ...
        cache_path = os.path.join(
            self.stvqa_dataroot,
            "cache",
            f"{task}_{split}_{str(self._max_seq_length)}"
