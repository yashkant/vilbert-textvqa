...
class SpatialBertSelfAttention(nn.Module):
    ...
    def __init__(self, config, use_implicit=False, use_regat=False):
        ...
        self.use_regat = use_regat
        if self.use_regat:
            self.relative_value_bias = nn.Embedding(182 * 182, 64)
            self.relative_attention_bias = nn.Embedding(182 * 182, 64)
    ...
    def forward(self, hidden_states, attention_mask, spatial_adj_matrix, head_mask=None):
        ...
        if self.use_regat:
            ...
            attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) + relative_scores
...
class SpatialBertLayer(nn.Module):
    def __init__(self, config, use_implicit=False, use_regat=False):
        ...
        self.attention = SpatialBertAttention(config, use_implicit, use_regat)
...
class BertSpatialEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.num_regat_layers = counter["r"]
        ...
        logger.info(f"Num Regat Layers: {self.num_regat_layers}")
        ...
        self.regat_layers = nn.ModuleList([SpatialBertLayer(config, False, True) for _ in range(self.num_regat_layers)])
    ...
    def forward(self, hidden_states, attention_mask, batch_dict, head_mask=None):
        ...
        regat_iter = iter(self.regat_layers)
