...
class SpatialBertSelfAttention(nn.Module):
    ...
    def __init__(self, config, use_implicit=False, use_regat=False):
        ...
        if self.use_regat:
            self.regat_attention_bias = nn.Embedding(12, 1)
            self.regat_value_bias = nn.Embedding(12, config.hidden_size)
    ...
    def forward(self, hidden_states, attention_mask, spatial_adj_matrix, head_mask=None):
        ...
        if self.use_regat:
            adj_matrix = (spatial_adj_matrix.argmax(dim=-1) * (spatial_adj_matrix.sum(dim=-1) > 0))
...
class BertSpatialEncoder(nn.Module):
    def __init__(self, config):
        ...
        logger.info(f"Num Implicit Layers: {self.num_regat_layers}")
