...
class BertOCRDecodeEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_type_embeddings_new = nn.Embedding(3, config.hidden_size)
    ...
    def forward(self, input_ids, decode_ids, vocab_cls_weights, ocr_features, ocr_bboxes, token_type_ids=None, task_ids=None, position_ids=None):
        ...
        token_type_ids[:, input_ids.size(1):input_ids.size(1) + ocr_features.size(1)] = 1
        token_type_ids[:, input_ids.size(1) + ocr_features.size(1):] = 2
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config, v_to_t_layer=None):
        ...
        self.img_embeddings = BertImageEmbeddings(config)
    ...
    def forward(
        ...
        v_embedding_output = self.img_embeddings(
            input_imgs,
...
class VILBertForVLTasks(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.training:
            ...
            num_dec_steps = batch_dict["train_prev_inds"].size(1)
            batch_dict["train_prev_inds"] = torch.zeros_like(batch_dict["train_prev_inds"])
            batch_dict["train_prev_inds"][:, 0] = registry.BOS_IDX
            assert num_dec_steps > 0
            # greedy decoding at test time
            for t in range(num_dec_steps):
                sequence_output_t, sequence_output_v, all_attention_mask = self.bert(
                    batch_dict,
                    output_all_encoded_layers=output_all_encoded_layers,
                    output_all_attention_masks=output_all_attention_masks,
                ...
                # decoding
                argmax_inds = textvqa_scores.argmax(dim=-1)
