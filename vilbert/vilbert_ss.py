...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, task_ids=None, position_ids=None):
        ...
        # import pdb
        # pdb.set_trace()
        # cls_indices = input_ids.new_ones(input_ids.size(0), 1) \
        #               * registry["bert_tokenizer"].encode(registry["bert_tokenizer"].cls_token)[0]
        # input_ids = torch.cat([input_ids, cls_indices], dim=1)
        # (YK): Join with a <CLS> token input decoding-step
...
class BertDecodeEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, decode_ids, vocab_cls_weights, ocr_features, token_type_ids=None, task_ids=None, position_ids=None):
        ...
        # Cannot use dropout with decoding steps!?
        embeddings = torch.cat((words_embeddings, decode_embeddings), dim=1)
        return embeddings
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config, v_to_t_layer=None):
        ...
        self.v_embeddings = BertImageOCREmbeddings(config)
    ...
    def forward(
        ...
        # add single-step mask
        # decoding_mask = attention_mask.new_zeros((attention_mask.size(0), 1))
        ...
        image_embeddings, ocr_embeddings = self.v_embeddings(
            input_imgs,
...
class VILBertForVLTasks(BertPreTrainedModel):
    ...
    def forward(
        ...
        del batch_dict["train_prev_inds"]
        batch_dict["targets"] = batch_dict["targets"][:, 0].unsqueeze(1)
        batch_dict["train_loss_mask"] = batch_dict["train_loss_mask"][:, 0].unsqueeze(1)
        sequence_output_t, sequence_output_v, all_attention_mask = self.bert(
            batch_dict,
            output_all_encoded_layers=output_all_encoded_layers,
            output_all_attention_masks=output_all_attention_masks,
        )
        textvqa_scores = self.build_textvqa_scores(batch_dict, sequence_output_t, sequence_output_v)
        return textvqa_scores
